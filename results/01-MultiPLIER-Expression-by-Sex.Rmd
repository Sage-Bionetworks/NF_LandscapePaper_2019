...
Here, we're evaluating the expression of latent variables in NF tumors as they pertain to the sex of the patient. These data were generated as part of developing the braiNFood app. 
...
download.file('https://github.com/Sage-Bionetworks/nf-lv-viz/raw/master/data/filt_nf_mp_res.feather', destfile = "./mp_res.feather")
mp_res<-read_feather('./mp_res.feather') %>% filter(sex != "NA", isCellLine != "TRUE")
...
res_model <- mp_res %>% 
  ...
  mutate(pval = map(data,ttest) %>% 
           ...
           round(., digits = 3)) %>% 
  filter(pval < 0.1) %>% 
  mutate(title = paste0(latent_var, " in ", modelOf, " BH p-value = ", pval) %>% 
           str_wrap(., width = 40)) %>% 
