...
ggplot(subset(full.tab,`Distance Measured`=='Batch'))+geom_boxplot(aes(y=`Pairwise Distance`,x=`Group`,fill=`Values Clustered`))+scale_y_log10()+theme_bw()+scale_fill_brewer(palette='Dark2')+ theme(axis.text.x=element_text(angle=45, hjust=1))
...
ggsave('withinTumorLVclustering.pdf')
```

## Best LV clustering
Lastly let's try to subset to only the top 40 best latent variables.
...
autoplot(prcomp(t(as.mat)),data=samp.data,colour="tumorType",shape='study')+ggtitle('LV-derived clustering')+theme_bw()+
  scale_color_manual(values = c("Cutaneous Neurofibroma"="#ca054d", 
                               ...
                               "Neurofibroma" = "#a4d4b4",
                               "Malignant Peripheral Nerve Sheath Tumor" = "#ffcf9c"))
ggsave('fig1panelC.pdf',useDingbats=FALSE)
```
